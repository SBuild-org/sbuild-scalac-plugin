...
import org.sbuild._
import org.sbuild.plugins.scalac.internal.ForkSupport

/**
 * Companion object for [[ScalacTask]], the Scala Compiler Task.
 *
 * Use [[ScalacTask$#apply]] to configure and execute it in one go.
 *
 */
object ScalacTask {

  /**
   * Configure and execute the Scalac task.
   *
   * For parameter documentation see the [[ScalacTask]] constructor.
   *
   */
  def apply(
...
/**
 * The [[http://www.scala-lang.org/ Scala]] [[http://www.scala-lang.org/docu/files/tools/scalac.html Compiler]] task.
 *
 * The compiler can be configured via constructor parameter or `var`s. To compile use [[ScalacTask#execute]].
 *
 * To easily configure and execute the compiler in one go, see [[ScalacTask$#apply]].
 *
 * '''Example:'''
 * {{{
 * import org.sbuild._
 *
 * @version("0.8.0.9010")
 * @classpath("mvn:org.sbuild.plugins:org.sbuild.plugins.scalac:0.0.9000")
 * class SBuild(implicit _project: Project) 
 * 
 *   // compile classpath
 *   val compileCp = ...
 *
 *   // The Scalac compiler classpath, here version 2.11.0
 ...
 *     org.sbuild.plugins.scalac.ScalacTask(
 ...
 *
 * @constructor
 * Create a new Scalac Compiler task instance. All parameters can be omitted and set later.
 *
...
class ScalacTask(
  ...
  private[this] val log = Logger[ScalacTask]
  ...
  protected def compileInternal(args: Array[String]): Int = {
    ...
    val compilerClassLoader = new URLClassLoader(compilerClasspath.map { f => f.toURI().toURL() }.toArray, classOf[ScalacTask].getClassLoader)
